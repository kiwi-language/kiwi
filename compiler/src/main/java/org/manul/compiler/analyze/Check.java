...
import org.manul.compiler.element.LocalVar;
...
import static org.manul.compiler.syntax.ModifierTag.*;
...
public class Check extends StructuralNodeVisitor {
    ...
    public Void visitAssignExpr(AssignExpr assignExpr) {
        ...
        var strConcat = assignExpr.lhs().getType().getUnderlyingType().isString() && assignExpr.op() == BinOp.ADD;
        if (!strConcat && !Types.isConvertible(assignExpr.rhs().getType(), assignExpr.lhs().getType())) {
