...
public class ManulEnv {
    ...
    private static Path getJvmStdLibPath() {
        try {
            var codeSource = ManulEnv.class.getProtectionDomain().getCodeSource();
            if (codeSource == null) {
            ...
            // Case B: Running from Classes (IDE / Unit Test / Maven)
                ...
                // Structure: .../manul/compiler/target/classes
                // Target:    .../manul/stdlib
                Path root = codeLocation.toPath()
                        .getParent() // target
                        .getParent() // compiler
                        .getParent(); // manul project root
