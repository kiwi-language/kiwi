...
public class ForwardedReferenceMarkingTask extends Task {
    ...
    public ForwardedReferenceMarkingTask(String id) {
        super("Forwarded reference marking task for " + id);
        this.id = id;
    }
    ...
    private void onTaskDone(IEntityContext context, Id id) {
        try {
            ...
            target.switchId();
            context.bind(new ReferenceForwardingTask(this.id));
        }
