...
public class TreeSetNative extends IterableNative {

    private final ClassInstance instance;
    private final TreeMap<ComparableKeyWrap, Integer> element2index = new TreeMap<>();
    ...
    public BooleanValue equals(Value o, CallContext callContext) {
        if(o instanceof Reference ref) {
            if(ref.resolve() instanceof ClassInstance that
                ...
                var thatNat = new TreeSetNative(that);
                var thatArray = thatNat.array;
                if(array.size() == thatArray.size()) {
                    for (Value value : thatArray) {
                        if(!element2index.containsKey(new ComparableKeyWrap(value, callContext)))
    ...
    public Value first(CallContext callContext) {
        if(element2index.isEmpty()) {
            throw new NoSuchElementException();
        }
        return element2index.firstKey().value();
    }
    ...
    public Value remove(Value value, CallContext callContext) {
        ...
        if (index != null) {
            ...
            if (index != lastIdx) {
                ...
                element2index.put(new ComparableKeyWrap(last, callContext), index);
            }
            return Instances.trueInstance();
        } else {
            return Instances.falseInstance();
    ...
    public Value contains(Value value, CallContext callContext) {
        return Instances.booleanInstance(element2index.containsKey(new ComparableKeyWrap(value, callContext)));
    }
    ...
    private void initializeElementToIndex(CallContext callContext) {
        for (int i = 0; i < array.getElements().size(); i++) {
            element2index.put(new ComparableKeyWrap(array.get(i), callContext), i);
        }
    }
