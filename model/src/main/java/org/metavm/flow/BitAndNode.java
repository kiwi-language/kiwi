...
public class BitAndNode extends NodeRT {
    ...
    public static BitAndNode save(NodeDTO nodeDTO, NodeRT prev, Code code, NodeSavingStage stage, IEntityContext context) {
        BitAndNode node = (BitAndNode) context.getNode(Id.parse(nodeDTO.id()));
        if (node == null)
            node = new BitAndNode(nodeDTO.tmpId(), nodeDTO.name(), prev, code);
        return node;
    }
