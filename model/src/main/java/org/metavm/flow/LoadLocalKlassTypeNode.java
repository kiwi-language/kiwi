...
public class LoadLocalKlassTypeNode extends Node {

    @SuppressWarnings("unused")
    private static Klass __klass__;
    private final KlassType type;

    public LoadLocalKlassTypeNode(@NotNull String name, @Nullable Node previous, @NotNull Code code, KlassType type) {
        super(name, StdKlass.klassType.type(), previous, code);
        this.type = type;
    }

    public static Node read(CodeInput input, String name) {
        return new LoadLocalKlassTypeNode(name, input.getPrev(), input.getCode(), (KlassType) input.readConstant());
    }
    ...
    public void writeContent(CodeWriter writer) {
        writer.write("ltlocalkt " + type.getTypeDesc());
    }

    @Override
    public int getStackChange() {
        return 1 - type.getKlass().getTypeParameters().size();
    }

    @Override
    public void writeCode(CodeOutput output) {
        output.write(Bytecodes.LT_LOCAL_KLASS);
    ...
    public <R> R accept(ElementVisitor<R> visitor) {
        return visitor.visitLoadLocalKlassTypeNode(this);
    }
