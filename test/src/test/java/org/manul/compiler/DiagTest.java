...
public class DiagTest extends TestCase {
    ...
    public void testConstructorNotFound() {
        ...
        assertEquals("""
                dummy.mnl:3: Cannot resolve function with given argument types: int, string
                        op1(1, "a")
                        ^""", diags.head().toString());
    }
    ...
    public void testChineseCharacter() {
        ...
        assertEquals(1, diags.size());
        assertEquals("""
                dummy.mnl:1: Unexpected token: 1
                    @Label("商品")1
                            　　  ^""",
                diags.head().toString()
        );
    ...
    public void testModifyCapturedVariable() {
        ...
        assertEquals(
                """
                        dummy.mnl:5: Cannot modify a captured variable
                                    values.forEach(i -> sum += i)
                                                        ^""",
    ...
    public void testDeletedInitParam() {
        ...
        assertEquals(
                """
                        dummy.mnl:2: Modifier 'deleted' is not allowed here
                                deleted var name: string
                                ^""",
                log.getDiags().head().toString()
        );
    ...
    public void testMisplacedIndexField() {
        ...
        assertEquals(
                """
                        dummy.mnl:3: Index field must be defined in the class it indexes
                                static val itemQuantityIdx = Index<int, Item>(false, i -> i.quantity)
                                           ^""",
                log.getDiags().head().toString()
        );
    ...
    public void testInvalidIndexValueType() {
        ...
        assertEquals(
                """
                        dummy.mnl:3: Invalid index value type
                                static val intIdx = Index<int, int>(false, i -> i)
                                           ^""",
                log.getDiags().head().toString()
        );
    ...
    public void testNonStaticIndexField() {
        ...
        assertEquals(
                """
                        dummy.mnl:3: Index field must be static
                                val nameIdx = Index<string, Product>(true, p -> p.name)
                                    ^""",
                log.getDiags().head().toString()
        );
    ...
    public void testReservedFieldNames() {
        ...
        assertEquals("""
                dummy.mnl:2: Reserved field name
                        val children = new any[]
                            ^""",
                log.getDiags().head().toString());
        assertEquals(
                """
                        dummy.mnl:1: Reserved field name
                            class Foo(val id: string, val parent: any) {
                                                          ^""",
    ...
    public void testSelfRefVariableInit() {
        ...
        assertEquals(1, log.getDiags().size());
        assertEquals("""
                dummy.mnl:2: Cannot resolve symbol
                        var a = a
    ...
    public void testIllegalCast() {
        ...
        assertEquals(1, log.getDiags().size());
        assertEquals("""
                dummy.mnl:6: Illegal cast from any[] to Product.SKU[]
                            return children as SKU[]
                                            ^""",
                log.getDiags().getFirst().toString());
    }
    ...
    public void testFieldNotInitialized() {
        ...
        assertEquals(1, log.getDiags().size());
        assertEquals("""
                dummy.mnl:2: Field not initialized
                        var passwordHash: string
                            ^""",
                log.getDiags().getFirst().toString()
        );
    ...
    public void testIllegalUseOfType() {
        ...
        assertEquals(1, log.getDiags().size());
        assertEquals("""
                dummy.mnl:4: Illegal use of type
                            return Lab
                                   ^""",
                log.getDiags().getFirst().toString()
                );
    ...
    public void testLtOperandTypeCheck() {
        ...
        assertEquals(1, log.getDiags().size());
        assertEquals("""
                dummy.mnl:4: Operator '<' cannot be applied to 'string', 'string'
                            return s1 < s2
                                   ^""",
                log.getDiags().getFirst().toString());
    }
    ...
    public void testAddOperandTypeCheck() {
        ...
        assertEquals(1, log.getDiags().size());
        assertEquals("""
                dummy.mnl:4: Operator '+' cannot be applied to 'any', 'any'
                            return s1 + s2
                                   ^""",
                log.getDiags().getFirst().toString());
    }
    ...
    public void testPrefixOpTypeCheck() {
        ...
        assertEquals(1, log.getDiags().size());
        assertEquals("""
                dummy.mnl:4: Operator '!' cannot be applied to 'string'
                            return !s
                                    ^""",
                log.getDiags().getFirst().toString());
    }
    ...
    public void testIncompatibleTypes() {
        ...
        assertEquals(1, log.getDiags().size());
        assertEquals("""
                dummy.mnl:4: Incompatible types: void cannot be converted to int
                            return set()
                                   ^""", log.getDiags().getFirst().toString());
    }
    ...
    public void testAiLint() {
        ...
        assertEquals(3, log.getDiags().size());
        assertEquals("""
                dummy.mnl:4: Variable name matches the plural form of a class name but its type is not the array type of the class, such confusing naming is not allowed
                        var labs: string
                            ^""",
                log.getDiags().getFirst().toString());
        assertEquals("""
                dummy.mnl:3: Variable name ending with 'Id' is not allowed
                        var labId: string,
                            ^""",
                log.getDiags().get(1).toString());
        assertEquals("""
                dummy.mnl:2: Variable name matches a class name but its type is not the class, such confusing naming is not allowed
                        var lab: string,
                            ^""",
    ...
    public void testIncompatibleLambdaBodyExpr() {
        ...
        assertEquals(1, log.getDiags().size());
        assertEquals("""
                dummy.mnl:3: Incompatible types: int[] cannot be converted to int
                        static val tagIdx = Index<int, Lab>(false, l -> l.tags)
                                                                          ^""",
                log.getDiags().getFirst().toString());
    }
    ...
    public void testDupVariableDef() {
        ...
        assertEquals(1, log.getDiags().size());
        assertEquals("""
                dummy.mnl:3: Variable 'name' is already defined in the scope
                        var name: string?
                            ^""",  log.getDiags().getFirst().toString());
    }
    ...
    public void testDupLocalVar() {
        ...
        assertEquals(1, log.getDiags().size());
        assertEquals("""
                dummy.mnl:9: Variable 'a' is already defined in the scope
                                var a = 1
                                    ^""", log.getDiags().getFirst().toString());
    }
    ...
    public void testDupMethod() {
        ...
        assertEquals(1, log.getDiags().size());
        assertEquals("""
                dummy.mnl:3: Function 'test(Class java.lang.String)' is already defined in the scope
                        fn test(s: string) {}
                           ^""", log.getDiags().getFirst().toString());
    }
    ...
    public void testCyclicInheritance() {
        ...
        assertEquals(1, log.getDiags().size());
        assertEquals("""
                dummy.mnl:1: Cyclic inheritance
                    class Lab: Lab {}
                               ^""", log.getDiags().getFirst().toString());
    }
    ...
    public void testMissingTokenValidator() {
        ...
        assertEquals(
                """
                        dummy.mnl:7: A TokenValidator bean must be defined in order to use @CurrentUser annotation
                                fn getCurrentUser(@CurrentUser user: User) -> User {
                                                  ^""",
                log.getDiags().getFirst().toString()
        );
