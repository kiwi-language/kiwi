...
////        Assert.assertEquals(field.getName(), request.fieldAdditions().getFirst().fieldName());
////        Assert.assertEquals(1, request.initializerKlasses().size());
////        Assert.assertEquals(fooKlass.getCodeNotNull() + "Initializer", request.initializerKlasses().getFirst().code());
...
//                var foo = new UpgradeFoo("foo", Instances.nullInstance());
//                foo.setValue(Instances.longInstance(1L));
//                context.bind(foo);
//                context.finish();
//                return foo.getId();
//            }
...
//        Assert.assertTrue(dc.contains(barKlass.getId()));
//
//        try(var context = newContext()) {
//            var foo = context.getEntity(UpgradeFoo.class, fooId);
//            Assert.assertEquals(new UpgradeBar("foo-bar").getReference(), foo.getBar());
//            var value = context.selectFirstByKey(UpgradeValue.IDX_FOO, foo.getReference());
//            Assert.assertNotNull(value);
//            Assert.assertEquals(Instances.longInstance(1L), value.getValue());
...
//
//        var defContext = new ReversedDefContext(sysDefContext);
...
//        var kInst = (ClassInstance) defContext.get(klassKlassId);
//        var k = defContext.getKlass(Klass.class);
//        Assert.assertSame(k, kInst.getInstanceKlass());
//
//        Assert.assertSame(
//                defContext.getKlass(Klass.class),
//                defContext.selectFirstByKey(Klass.UNIQUE_QUALIFIED_NAME, Instances.stringInstance(klassKlass.getQualifiedName())));
//
...
//                if(!e.isEphemeral()) {
//                    Assert.assertTrue(defContext.containsInstance(e));
//                }
//            });
//        }
//        defContext.close();
...
//        TestUtils.doInTransactionWithoutResult(() -> bootstrap1.save(false));
//        try(var context = newContext()) {
//            var singleton = context.selectFirstByKey(UpgradeSingleton.IDX_ALL_FLAGS, Instances.trueInstance());
...
//        var sysDefContext = ModelDefRegistry.getDefContext();
//        var klassKlassInst = sysDefContext.getKlass(Klass.class);
//        var instancePOs = List.of(Instances.toPO(klassKlassInst, ROOT_APP_ID));
//        var wal = new WAL(ROOT_APP_ID);
