...
public class MemInstanceSearchServiceV2Test extends TestCase {
    ...
    public void test() {
        ...
        assertEquals(Objects.requireNonNull(foo.getId()).getPhysicalId(), (long) result.data().get(0));
    }
    ...
    public void testSearchingSuperClassField() {
        ...
        var result = memInstanceSearchServiceV2.search(new SearchQuery(
                TestConstants.APP_ID,
                Set.of(livingBeingTypes.humanType().getId()),
                Expressions.eq(
                        Expressions.property(
        ...
        assertEquals(1, result.total());
        assertEquals(Objects.requireNonNull(human.getId()).getPhysicalId(), (long) result.data().get(0));
