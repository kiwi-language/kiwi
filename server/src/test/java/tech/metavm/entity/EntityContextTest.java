...
public class EntityContextTest extends TestCase {
    ...
    public void testGet() {
        ...
        fooInst.initId(idProvider.allocateOne(ROOT_APP_ID, fooType));
        instanceContext.replace(fooInst);
    ...
    public void test_remove() {
        ...
        TaskSignal signal = rootContext.selectByUniqueKey(TaskSignal.IDX_APP_ID, APP_ID);
    ...
    public void testSelectByKey() {
        ...
        instanceStore.addIndex(
                APP_ID,
                PersistenceUtils.toIndexKeyPO(constraint.createIndexKey(List.of(fooInst.getStringField(fooNameField)))),
                fooInst.getId()
        );
    ...
    public void test_init_id_manually() {
        ...
        context.bind(foo);
        long fooId = idProvider.allocateOne(APP_ID, fooType);
    ...
    public EntityContext newIntegratedContext() {
        return newIntegratedContext(APP_ID);
    }

    public EntityContext newIntegratedRootContext() {
        return newIntegratedContext(ROOT_APP_ID);
    }

    public EntityContext newIntegratedContext(long appId) {
        return (EntityContext) instanceContextFactory.newContext(appId).getEntityContext();
    }
    ...
    private EntityContext newContext(IEntityContext parent) {
        MemInstanceContext instanceContext = new MemInstanceContext(
                APP_ID, idProvider, instanceStore, parent.getInstanceContext()
        );
