...
public class MockUtils {
    ...
    public static FooType createFooType(boolean initIds) {
        ...
        var fooNameField = FieldBuilder.newBuilder("name", "name", fooType, StandardTypes.getStringType()).build();
        var barType = ClassTypeBuilder.newBuilder("Bar", "Bar").build();
        var barCodeField = FieldBuilder.newBuilder("code", "code", barType, StandardTypes.getStringType()).build();
        var barArrayType = new ArrayType(null, barType, ArrayKind.CHILD);
        var fooBarsField = FieldBuilder.newBuilder("bars", "bars", fooType, barArrayType).build();
        if(initIds) {
            fooType.initId(1001L);
            barType.initId(1002L);
            barArrayType.initId(1003L);
            fooNameField.initId(2001L);
            fooBarsField.initId(2002L);
            barCodeField.initId(2003L);
        }
        return new FooType(fooType, barType, barArrayType, fooNameField, fooBarsField, barCodeField);
    }
