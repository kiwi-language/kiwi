...
public class ManulServerManager {

    private static final String APP_NAME = "ManulServer";
    ...
    // Configurable paths
    private static final Path CONFIG_FILE = APP_ROOT_DIR.resolve("conf").resolve("manul.yml");
    ...
    private static Path getJarPath() {
        try {
            var codeSource = ManulServerManager.class.getProtectionDomain().getCodeSource();
            if (codeSource == null) {
            ...
            if (codeLocation.isFile() && codeLocation.getName().endsWith(".jar")) {
                // Structure: .../manul/bin/compiler.jar
                // Target:    .../manul/pkg/stdlib
            ...
            throw new RuntimeException("Failed to locate Manul Standard Library", e);
        }
    }
