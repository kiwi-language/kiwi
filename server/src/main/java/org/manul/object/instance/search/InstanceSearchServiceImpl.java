...
public class InstanceSearchServiceImpl implements InstanceSearchService {
    ...
    private void createPhysicalIndex(String indexName) throws IOException {
        var resp = restClient.performRequest(new Request("HEAD", "/" + indexName));
        var code = resp.getStatusLine().getStatusCode();
        if (code == HttpStatus.SC_OK) {
            ...
            throw new IOException("Failed create Elasticsearch index. Response code: " + code);
    }
}