...
import tech.metavm.management.persistence.BlockMapper;
import tech.metavm.management.persistence.BlockPO;
import tech.metavm.management.persistence.RangeInc;
...
public class MemBlockMapper implements BlockMapper {
    ...
    @Override
    public List<BlockPO> selectActiveRanges(long tenantId, Collection<Long> typeIds) {
        return NncUtils.flatMapAndFilter(
                typeIds,
                typeId2blocks::get,
                block -> block.getTenantId() == tenantId && Boolean.TRUE.equals(block.getActive())
        );
    }
}
