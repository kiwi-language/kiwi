...
public class ReferenceCleanupJobTest extends TestCase {
    ...
    public void test() {
        ...
        ReferenceCleanupJob job = new ReferenceCleanupJob(qux.getId(), qux.getType().getName(), qux.getTitle());
        context = newContext();
        ...
        job = entityContext.getEntity(ReferenceCleanupJob.class, job.getId());
        job.run(context);
        context.finish();
