...
public class TranspileUtil {
    ...
    public static String unescapeStringBlock(String s) {
        var textStart = 1 + Math.min(
                (s.indexOf('\r') + s.length()) % s.length(),
                (s.indexOf('\n') + s.length()) % s.length()
        );
        return s.substring(textStart).replaceAll("\\\\[\r\n]", "");
    }

    public static String unescapePerlString(String oldstr) {
