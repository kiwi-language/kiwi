...
public class ProxyGenerator extends AbstractGenerator {
    ...
    private void generateContr() {
        ...
        if (transactional) {
            if (!constr.getParameters().isEmpty())
                write(", ");
            write("org.manul.jdbc.TransactionOperations __transactionOps__");
        }
        if (scheduled) {
            if (!constr.getParameters().isEmpty() || transactional)
                write(", ");
            write("org.manul.schedule.Scheduler __scheduler__");
        }
        writeln(") {");
        indent();
        write("super(");
    ...
    private void writeImports() {
        writeImport("org.manul.server.Route");
        writeImport("org.manul.server.RouteConfig");
        writeImport("org.manul.server.HttpMethod");
        writeImport("org.manul.context.http.ResponseEntity");
        writeImport("org.jsonk.Type");
        writeImport("java.util.List");
        writeImport("org.manul.server.RequestUtil");
        writeln();
    }
    ...
    private void generateTransactional(ExecutableElement method) {
        ...
        write("), ")
                ...
                .write("org.manul.context.sql.TransactionPropagation.").write(tx.propagation().name())
                .writeln(");");
        deIndent();
        writeln("}");
        writeln();
